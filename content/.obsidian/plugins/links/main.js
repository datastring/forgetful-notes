...
__export(main_exports, {
  ObsidianLinksSettingTab: () => ObsidianLinksSettingTab,
...
var LinkData = class extends TextPart {
  constructor(type, content, position, link, text) {
  ...
  if (linkType & 2 /* Wiki */) {
    while (match = wikiLinkRegEx.exec(text)) {
      if (startPos >= match.index && endPos <= wikiLinkRegEx.lastIndex) {
        ...
        if (text2 !== void 0) {
          const textIdx = text2 ? raw.indexOf(text2, (linkData.link ? linkData.link.position.end : raw.indexOf("|")) + 1) : raw.length - 2;
  ...
  if (linkType & 4 /* Html */) {
    while (match = htmlLinkRegEx.exec(text)) {
      if (startPos >= match.index && endPos <= htmlLinkRegEx.lastIndex) {
        const [raw, url, text2] = match;
        const linkData = new LinkData(4 /* Html */, raw, new Position(match.index, htmlLinkRegEx.lastIndex));
        if (url) {
          const linkIdx = raw.indexOf(url);
          linkData.link = new TextPart(url, new Position(linkIdx, linkIdx + url.length));
        }
  ...
  if (linkType & 8 /* Autolink */) {
    while (match = angleBracket.exec(text)) {
      if (startPos >= match.index && endPos <= angleBracket.lastIndex) {
        const [raw, url] = match;
        const linkData = new LinkData(8 /* Autolink */, raw, new Position(match.index, angleBracket.lastIndex));
        if (url) {
          const linkIdx = raw.indexOf(url);
          linkData.link = new TextPart(url, new Position(linkIdx, linkIdx + url.length));
        }
...
function removeLinksFromHeadings(text) {
...
var LinkTextSuggest = class extends import_obsidian.EditorSuggest {
  ...
  selectSuggestion(suggestion) {
    ...
    if ((linkData == null ? void 0 : linkData.type) == 2 /* Wiki */) {
      ...
      if (linkData.text) {
        editor.replaceRange("|" + suggestion, editor == null ? void 0 : editor.offsetToPos(textStartOffset), editor == null ? void 0 : editor.offsetToPos(textStartOffset + 1));
      } else {
    ...
    } else if (((_c = this.suggestContext.linkData) == null ? void 0 : _c.type) == 1 /* Markdown */) {
      const editor = this.context.editor;
      const textStartOffset = ((_d = this.suggestContext.linkData) == null ? void 0 : _d.position.start) + 1;
      editor.replaceRange(suggestion, editor == null ? void 0 : editor.offsetToPos(textStartOffset));
      editor.setSelection(editor.offsetToPos(textStartOffset), editor.offsetToPos(textStartOffset + suggestion.length));
...
var ObsidianLinksPlugin = class extends import_obsidian4.Plugin {
      ...
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        ...
        if (linkData) {
          ...
          if (linkData.text && linkData.text.content.length > 0) {
            menu.addItem((item) => {
              item.setTitle("Edit link text").setIcon("text-cursor-input").onClick(async () => {
          ...
          if (linkData.link) {
            menu.addItem((item) => {
              item.setTitle("Copy link destination").setIcon("copy").onClick(async () => {
        ...
        if (linkData) {
          ...
          if (this.createLinkFromSelectionHandler(editor, true)) {
            menu.addItem((item) => {
              item.setTitle("Create link").setIcon("link").onClick(async () => {
          ...
          if (this.createLinkFromClipboardHandler(editor, true)) {
            menu.addItem((item) => {
              item.setTitle("Create link from clipboard").setIcon("link").onClick(async () => {
  ...
  copyLinkUnderCursorToClipboard(linkData) {
    ...
    if (linkData == null ? void 0 : linkData.link) {
      navigator.clipboard.writeText((_a = linkData.link) == null ? void 0 : _a.content);
      new import_obsidian4.Notice("Link destination copied to your clipboard");
    }
  }
  removeLinksFromHeadingsHandler(editor, checking) {
    const selection = editor.getSelection();
    if (selection) {
      if (checking) {
  ...
  editLinkTextUnderCursorHandler(editor, checking) {
    const linkData = this.getLink(editor);
    if (checking) {
  ...
  async addLinkText(linkData, editor) {
    var _a, _b;
  ...
  createLinkFromSelectionHandler(editor, checking = false) {
    const selection = editor.getSelection();
    if (checking) {
      return !!selection;
    }
  ...
  onEditorPaste(evt, editor, view) {
    ...
    if (html && html.indexOf("<a") > 0) {
      const markdown = (0, import_obsidian4.htmlToMarkdown)(html);
  ...
  createLinkFromClipboardHandler(editor, checking = false) {
    if (checking) {
      return true;
    }
    (async () => {
...
var ObsidianLinksSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName("Title separator").setDesc("String used as headings separator in 'Add link text' command.").addText((text) => text.setValue(this.plugin.settings.titleSeparator).onChange(async (value) => {
